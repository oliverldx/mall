...
public class TrsCourseActivityController {
    ...
    @ResponseBody
    public CommonResult<CommonPage<TrsCourseActivityDto>>
        ...
        List<TrsCourseActivityDto> trsCourseActivityList = trsCourseActivityDao.getList(trsCourseActivityQueryParam);
     ...
     public CommonResult update(@PathVariable Long id,@Validated @RequestBody TrsCourseActivity trsCourseActivity,
        ...
        int count = 0;
        count = trsCourseActivityMapper.updateByPrimaryKeySelective(trsCourseActivity);
        if (count > 0) {
            return CommonResult.success(count);
        }
        return CommonResult.failed();
     ...
     public CommonResult delete(@RequestParam("ids") List<Long> ids) {
        ...
        for (Long id : ids) {
            count = trsCourseActivityMapper.deleteByPrimaryKey(id);
        }
        if (count > 0) {
            return CommonResult.success(count);
        }
    ...
    @ResponseBody
    public CommonResult<CommonPage<TrsCourseActivitySubListQueryParam>> courseList(@RequestParam(value = "pageSize", defaultValue = "4") Integer pageSize,
        TrsCourseActivitySubListQueryParam trsCourseActivitySubListQueryParam,
    @RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum) {
        PageHelper.startPage(pageNum,pageSize);
        List<TrsCourseActivitySubListQueryParam> courseList = trsCourseActivityDao.getCourseList(trsCourseActivitySubListQueryParam);
        return  CommonResult.success(CommonPage.restPage(courseList),"获取课程活动-course列表成功");
    }

    @ApiOperation("添加课程活动-course")
    @RequestMapping(value = "/addCourseList", method = RequestMethod.POST)
    @ResponseBody
    public CommonResult addCourseList(@RequestParam("ids") List<Long> ids, @RequestParam("activityId") Long activityId) {
        int count = 0;
        if(ids == null || ids.isEmpty()) {
            return CommonResult.failed();
        }
        Date createDate = new Date();
        for (Long id : ids) {
            TrsCourseActivity trsCourseActivity = new  TrsCourseActivity();
            trsCourseActivity.setCreateDate(createDate);
            trsCourseActivity.setModifyDate(createDate);
                    trsCourseActivity.setTrsCourseId(id);
                    trsCourseActivity.setTrsActivityId(activityId);
            count = trsCourseActivityMapper.insert(trsCourseActivity);
        }
        if (count > 0) {
            return CommonResult.success(count);
        }
